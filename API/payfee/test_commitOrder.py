...
class TestCommitOrder:
    ...
    def test_commitorder(self):
        ...
        jpara = json.dumps(para)
        print(para['login'])
        # jpara = json.dumps(para['json_newSeeCommitOrderVO'])
        # print(type(para['json_newSeeCommitOrderVO']["data"]))
        # print(para)
        req = requests.post(url, data=jpara, verify=False)
        print(req.json())
